...
extractBenchmarkData <- function(benchmarkDesign,
  ...
  benchmarkData <- checkmate::makeAssertCollection()
  ...
  checkmate::assertLogical(createStudyPopulation, null.ok = FALSE, len = 1, add = benchmarkData)

  checkmate::reportAssertions(benchmarkData)
  ...
  for (i in seq_along(minimalCohorts)) {
    ...
    if (!plpDataExists) {
      ...
      plpDataRaw <- PatientLevelPrediction::getPlpData(
  ...
  if (createStudyPopulation) {
    ...
    for (i in seq_along(benchmarkDesign)) {
      ...
      if (!populationExists) {
        ...
        studyPopulationRaw <- PatientLevelPrediction::createStudyPopulation(
          plpData = plpDataRaw,
