...
We will consider a sample of the target cohort for each problem. [@John2022] has shown through learning curves  that a stratified sample of the original population, containing between 2500-3500 positive outcome events, is adequate to generate similar out-of-sample c-statistic as using the whole population. Use of a larger sample size to enhance discriminative performance comes with lower gains in discrimination and the cost of longer running times. 

The sample will be estimated by first estimating the study population for each problem given the specification of each problem (e.g. outcome occurred within the specified time-at-risk). From this population we will estimate the outcome rate (outcome cases/all people in target cohort) and use this rate to estimate the necessary sample size
with a pre-specified number of outcomes (e.g. 2500), in order to reduce running times.     
